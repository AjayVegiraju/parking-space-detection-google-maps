...
def process_image():
    ...
    for bbox in detections.xyxy:
        ...
        cv2.drawMarker(img, (x_center, y_center), color=(0, 255, 0), markerType=cv2.MARKER_CROSS, markerSize=20,
                       thickness=2)
    ...
    return jsonify({
        'annotated_image': 'data:image/png;base64,' + base64.b64encode(open(annotated_image_path, 'rb').read()).decode(
            'utf-8'),
        'incoming_image': 'data:image/png;base64,' + base64.b64encode(open(incoming_image_path, 'rb').read()).decode(
            'utf-8')
    })
